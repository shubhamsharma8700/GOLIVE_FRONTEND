...
//   const { data: eventsData } = useListEventsQuery({ limit: 200 });
...
//     { eventId: 'all', title: 'All Events' },
//     ...apiEvents.map((e: { eventId: string; title: string }) => ({ eventId: e.eventId, title: e.title || 'Untitled' })),
//   ], [apiEvents]);

//   const filteredTopEvents = useMemo(() => {
//     return topEvents.filter(() => true);
//   }, []);

//   const handleEventFilterChange = (value: string) => {
//     setSelectedEventId(value);
//     getAnalyticsReport(value === 'all' ? {} : { eventId: value }).catch(() => {});
//   };

//   return (
//     <div className="space-y-6">
//       {/* Event filter dropdown - events from API */}
//       <div className="relative max-w-2xl">
//         <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
//         <Select value={selectedEventId} onValueChange={handleEventFilterChange}>
...
//                   <th className="text-left p-4 text-sm text-[#B89B5E]">Type</th>
...
//                 {apiEvents.length === 0 ? (
//                   <tr>
//                     <td colSpan={4} className="p-6 text-center text-[#6B6B6B]">No events found</td>
//                   </tr>
//                 ) : (
//                   apiEvents.map((event: { eventId: string; title: string; startTime?: string; eventType?: string; status?: string; vodStatus?: string }) => (
//                     <tr key={event.eventId} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
...
//                           : '—'}
//                       </td>
//                       <td className="p-4 text-[#6B6B6B]">{event.eventType ?? '—'}</td>
//                       <td className="p-4">
//                         <Badge variant="secondary">{event.status ?? event.vodStatus ?? '—'}</Badge>
//                       </td>
//                     </tr>
//                   ))
//                 )}
